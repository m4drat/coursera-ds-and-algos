...
int32_t BinarySearch(const std::vector<int32_t>& array, int32_t elementToFind) {
    ...
    while (left <= right) {
        int32_t mid = static_cast<int32_t>(std::floor(left + static_cast<float>(right - left) / 2.0f));
...
int32_t LinearSearch(const vector<int32_t>& array, int32_t elementToFind) {
...
void CheckSolution() {
    ...
    for (const auto& testcase : problemSolutionPairs) {
        ...
        if (answer != testcase.answer) {
            throw std::runtime_error("Got: " + std::to_string(answer) + ". Expected: " + std::to_string(testcase.answer));
        }
    }
}
